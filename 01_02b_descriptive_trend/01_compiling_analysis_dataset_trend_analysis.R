...
#get countries with recent survey posiives greater than 1%
df_recent <- read.csv(file.path(PopDir, "analysis_dat/final_surveys.csv")) %>% mutate(cntryId = stringr::str_extract(code_year, "^.{2}"))
...
ir_urban<- ir_urban %>%map(~dplyr::select(., colnames(ir_urban[[13]])))
ir_urban <- plyr::ldply(ir_urban)


ir_rural %>% map(~dim(.x)[[2]]) #get smallest column length in list and position
ir_rural<- ir_rural %>%map(~dplyr::select(., colnames(ir_rural[[13]])))
...
pr_urban<- pr_urban %>%map(~dplyr::select(., colnames(pr_urban[[13]])%>% discard(~ .x %in% c("hc11", "hc10", "hc12"))))
...
pr_rural<- pr_rural %>%map(~dplyr::select(., colnames(pr_rural[[13]])%>% discard(~ .x %in% c("hc11", "hc10", "hc12"))))
