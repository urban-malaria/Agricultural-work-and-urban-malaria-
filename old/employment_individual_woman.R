...
DataDir <-file.path(Drive, "Downloads", 'multi_agric_data')
...
## 
dhs <- read.files("*MLIR.*\\.DTA", DataDir, read_dta, encoding = "latin1")
#dhs <- read.files(DataDir, "*ZMIR.*\\.DTA", 'NGIR7AFL', read_dta) #reads in the IR files
...
## 
read.files("*ZMPR.*\\.DTA", DataDir, read_dta, encoding = "latin1")
dhs_pr <- read.files(DataDir, "*NGPR.*\\.DTA", 'NGPR7AFL', read_dta) #reads in the PR files

# Estimating malaria status by microscopy 
dhs_pr_vars <- dhs_pr %>% map(~filter(., hv042 == 1 & hv103 == 1 & hc1 %in% c(6:59) & hml32 %in% c(0, 1),hv025 == 1)) %>%
...
dhs_kr_vars <- dhs_kr %>% map(~filter(., b5==1, b19 < 60, h22 == 1, v025 == 1)) %>%
